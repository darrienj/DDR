...
public class Score {
	...
	private static String[] descriptionName = {"Marvelous","Perfect","Great","Good","Almost","Miss"};
	private Song song;
	...
	public Score(int id,Dimension scaleFactor,Song song) throws IOException{
		System.out.println(song.getNoteCount());
		this.id = id;
		this.song = song;
		...
		for(int i = 0;i<description.length;i++){
			for(int j = 0;j<3;j++){
				...
				int width = (int)((height*1.0/img.getHeight())*img.getWidth());
		...
		//scale grade images
		BufferedImage[][] grades = {GRADE_S,GRADE_A,GRADE_B,GRADE_C,GRADE_D,GRADE_F};
		...
		NOTE_AMOUNT = BASESCORE/(song.getNoteCount() + song.getHoldCount()/16.0); //1 hold beat = 1 note
	...
	public BufferedImage[] missNote(){
		return addNote(1000);
	}
	public BufferedImage[] addNote(double difference){
		combo++;
		this.totalNotes++;
		double cMult = 1;
		difference -= (Math.min(this.song.getBpm()-90,90)/90.0)*.4*difference;//0-90
		if(difference > 1.5){
			combo = 0;
			 cMult = 0;
			noteCount[MISS]++;
		...
		return getDescription(difference);
	}
	public BufferedImage[] getDescription(double difference){
		if(difference > 1.5){
			return description[MISS];
		} else if(difference > .8){
			return description[ALMOST];
		} else if(difference > .45){
			return description[GOOD];
		} else if(difference > .2){
			return description[GREAT];
		} else if(difference > .08){
			return description[PERFECT];
		} else{
			return description[MARVELOUS];
		}
	}
	public int getScore(){
		return (int)score;
	...
	public void saveHighScore() throws IOException{
		...
		for(int line = 0;line < highScore.length;line++){
			...
			for(int index = 0;index<highScore[line].length;index++){
				tmp +=highScore[line][index].getName()+", " + highScore[line][index].getScore()+", ";
			}
			ans +=tmp;
