...
public class SongSelectorPanel extends JPanel{
	...
	public SongSelectorPanel(Dimension scaleFactor,String[][] songList,SelectorBackgroundPanel sbp){
		...
		try{
			...
			Image tmp = img.getScaledInstance((int)(1280*scaleFactor.getWidth()*.4*.9) -2*borderWidth,(int)( 800*scaleFactor.getHeight()*(1.0/SONGS_ON_SCREEN) - 2*borderWidth), BufferedImage.TYPE_INT_RGB);
			img = new BufferedImage(tmp.getWidth(null),tmp.getHeight(null),BufferedImage.TYPE_INT_RGB);
			Graphics g = img.getGraphics();
			g.drawImage(tmp, 0, 0, img.getWidth(),img.getHeight(), null);
			
			highlight = ImageIO.read(new File(Constants.SELECT_GRADIENT_HIGHLIGHTED));
			tmp = highlight.getScaledInstance((int)(1280*scaleFactor.getWidth()*.4*.9) -2*borderWidth,(int)( 800*scaleFactor.getHeight()*(1.0/SONGS_ON_SCREEN) - 2*borderWidth), BufferedImage.TYPE_INT_RGB);
			highlight = new BufferedImage(tmp.getWidth(null),tmp.getHeight(null),BufferedImage.TYPE_INT_RGB);
			g = highlight.getGraphics();
			g.drawImage(tmp, 0, 0, highlight.getWidth(),highlight.getHeight(), null);
			
		} catch(Exception e){
			img = null;
			highlight = null;
		}
		int pageHeight = (int)((800*scaleFactor.getHeight()*(1.0/SONGS_ON_SCREEN) -4)*(songList.length+SONGS_ON_SCREEN));  //-4 to account for the border on the border
		this.setPreferredSize(new Dimension((int)(1280*scaleFactor.getWidth()*.4*.9),pageHeight));
		...
		for(int index = 0;index<songList.length+SONGS_ON_SCREEN;index++){
			...
			if(i >= songList.length){
				...
				panel = new SongPanel(Math.min(scaleFactor.getWidth(),scaleFactor.getHeight()),img,borderWidth,songList[i][0],false,songList[i],i % songList.length);
			}
		...
		this.addKeyListener(new DancePanelInput(sbp));
	...
	public int setSelection(int id){
		...
		this.requestFocus();
		this.selection = id;
		this.selection = this.selection < SMALLEST_SELECT ? this.songList.length + this.selection : this.selection;
